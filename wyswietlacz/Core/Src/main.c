...
void ShowInfo(char line1[], int speed, char prefix[]) {
	static int string_len = 16;

	char rpm[] = "";

	sprintf(rpm, prefix, speed);
	if (strlen(rpm) < string_len) {
		string_len = strlen(rpm);
		clear();
	}

	DisplayActualInfo(line1, rpm);
}
